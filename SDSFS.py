    ...
    def diffusion():
        for ag in population:
             if (ag.active==False):
                ...
                another_agent= np.random.choice(population) #agent A
                while(another_agent==ag):
                    another_agent=np.random.choice(population) #Agent B
...
x_norm = my_data[:,0:-1] / my_data[:,0:-1].max(axis=0) #normalising data before training, along a column using axis = 0
#print(x_norm)
