...
Implemented FS-SDS using Logistic Regression algorithm
Updated by: Mohammad Majid al-Rifaie, November 2020
...
def SDS(features_size, cost_function, data, maxIteration=50, population_size=100):  # agents=populan size=100
...
# Fintess function
def lr_accuracy(data, additional_metrics=False):
    np.random.shuffle(data)
    np.random.shuffle(data)
    train, test = train_test_split(data, test_size=0.2)
    model = LogisticRegression(solver='liblinear', random_state=0)
    model.fit(train[:, 0:-1], train[:, -1])
    accurancy = model.score(test[:, 0:-1], test[:, -1])
...
my_data = genfromtxt('ionosphere.csv', delimiter=',')  # delimiter-The string used to separate values.
...
# For all features
print("Accuracy for all features: " + str(abs(lr_accuracy(my_data, True))) + "\n")
...
print("Selected features " + str(SDS(features_size, lr_accuracy, my_data, maxIteration, population_size)))
